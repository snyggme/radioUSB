...
Int8U Data;
  ...
  NVIC_ClrPend(NVIC_USART);  // Clear interrupt in NVIC.
}
...
uint32_t Pclk, uartdiv=1;
    ...
    Pclk = CLK_GetClock(CLK_MAIN);
    while( (Pclk/uartdiv) > 70MHZ)
    {
      uartdiv ++;
    }
    Pclk /= uartdiv;
    // Enable Clocks
    CLK_SetUARTClk(uartdiv);
